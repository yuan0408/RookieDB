...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> optional = child.put(key, rid);

        if (optional.isPresent()) {
            Pair<DataBox, Long> dataBoxLongPair = optional.get();
            DataBox first = dataBoxLongPair.getFirst();
            Long second = dataBoxLongPair.getSecond();

            //find putting position
            int i = numLessThanEqual(first, keys);
            keys.add(i, first);
            //pointer need +1
            ...
            if (keys.size() > 2 * d) {
                ...
                //the pointer of rm2parent
                nChildren.add(children.remove(d + 1));
                while (d + 1 < keys.size()) {
                    nKeys.add(keys.remove(d + 1));
                    nChildren.add(children.remove(d + 1));
                }
                DataBox rm2parent = keys.remove(d);

                InnerNode nNode = new InnerNode(metadata, bufferManager, nKeys, nChildren, treeContext);
                optional = Optional.of(new Pair<>(rm2parent, nNode.getPage().getPageNum()));
            } else {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (keys.size()>2*d){
            ...
            DataBox mv2parent = keys.remove(d);

            InnerNode nNode = new InnerNode(metadata, bufferManager, nKeys, nChildren, treeContext);
            return Optional.of(new Pair<>(mv2parent,nNode.getPage().getPageNum()));
        }
