...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (optional.isPresent()) {
            ...
            nKeys.add(optional.get().getFirst());
            nChildren.add(root.getPage().getPageNum());
            nChildren.add(optional.get().getSecond());
            InnerNode nRoot = new InnerNode(metadata, bufferManager, nKeys, nChildren,lockContext);
            updateRoot(nRoot);
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            ...
            this.currentIterator = leafNode.scanAll();
        }

        public BPlusTreeIterator(DataBox key) {
            this.leafNode = root.get(key);
            this.currentIterator = leafNode.scanGreaterEqual(key);
        }
